...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (!stageTesting)
		{
			...
			if (!PlayStateChangeables.Optimize)
			{
				...
				black3 = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 4, FlxG.height * 2, FlxColor.BLACK);
			...
			if(SONG.song == 'Horrifying-Holidays')
			{
				black3.visible = true;
				black3.screenCenter();
				add(black3);
			...
			if (!PlayStateChangeables.Optimize)
				switch (Stage.curStage)
				{
					...
					case 'fireplace':
						...
						dad.scrollFactor.set(0.925, 0.925);
						boyfriend.scrollFactor.set(0.925, 0.925);
						boyfriend.x = 1133;
						boyfriend.y = 750;
						gf.x = 640;
						gf.y = 387;
		...
		if(SONG.song != 'Fractured' || SONG.song != 'Escape' || SONG.song != 'Disintegrate' || SONG.song != 'Stitched')
		...
		if (startTime != 0)
			{
				...
				for(i in 0...notes.members.length)
				{
					...
					else 
					{
						switch(curSong)
						{
							case 'Fractured' | 'Escape' | 'Disintegrate' | 'Stitched':
							{ 
								if(i == 3 || i == 4)
								{
         ...
         switch (SONG.player2)
           {
			 ...
			 case 'obama':
			  healthBar.createFilledBar(0xFFFFFFFF, 0xFF0097C4);
			 case 'padoru':
			  healthBar.createFilledBar(0xFFFFC0CB, 0xFF0097C4);
            }
        }
        else
		...
		vignette = new FlxSprite(0,0).loadGraphic(Paths.image('vignetteholiday', 'shared'));
	...
	var vignette:FlxSprite; //for horrifying-holidays, can be reused

	var debounce:Bool =  false; //using it for events my lil personal thing
	function ghoulAppear(weirdThing:Bool)
	{
		if(!debounce)
		{
			debounce = true;
			...
			{
				remove(dad);
				dad = new Character(dad.x, dad.y - 85, "ghoul");
				dad.scale.set(1, 1);
				dad.scrollFactor.set(0.925, 0.925);
				add(dad);
				Stage.camZoom = 0.7;
			...
			{
				remove(dad);
				dad = new Character(dad.x, dad.y + 85, "padoru");
				dad.scrollFactor.set(0.925, 0.925);
				dad.scale.set(1.75, 1.75);
				add(dad);
				...
				remove(iconP2);
				remove(healthBar);
				healthBar.createFilledBar(0xFF5A07F5, 0xFF0097C4); //dad colour for now
				iconP2.changeIcon(dad.curCharacter);
				add(healthBar);
				add(iconP2);
			}
	...
	override public function update(elapsed:Float)
	{
		...
		switch(curSong)
		{
			...
			case 'Horrifying-Holidays':
			{
				if(Conductor.songPosition > 15270 && Conductor.songPosition < 15400)
				{
					black3.visible = false;
				}
				if((Conductor.songPosition > 104730 && Conductor.songPosition < 105000))
				{
					ghoulAppear(true);
				...
				if((Conductor.songPosition > 123950 && Conductor.songPosition < 124200))
				{
					ghoulAppear(false);

					FlxTween.tween(vignette, {alpha: 0}, 0.2, {onComplete: function(twn:FlxTween){
						remove(vignette);
					}});
		...
		if (generatedMusic && currentSection != null)
		{
			...
			if ((camFollow.x != dad.getMidpoint().x + 150 + dadXOffset || camFollow.y != dad.getMidpoint().y - 100 + dadYOffset) && !currentSection.mustHitSection)
			{
				var offsetX = 0;
				var offsetY = 0;
				#if cpp
			...
			if (currentSection.mustHitSection && (camFollow.x != boyfriend.getMidpoint().x - 100 + bfXOffset || camFollow.y != boyfriend.getMidpoint().y - 100 + bfYOffset))
			{
				var offsetX = 0;
				var offsetY = 0;
				#if cpp
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		notes.forEachAlive(function(daNote:Note)
		{
			...
			if (daNote.mustPress && daNote.rating == "sick" || (diff > 0 && daNote.mustPress))
			{
				...
				if(daNote.noteType == 0 || daNote.noteType == 2 || daNote.noteType == 5 || daNote.noteType == 6)
					if (loadRep)
					{
